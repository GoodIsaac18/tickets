...
from .backup import auto_backup_diario
from .validators import InputValidator

# Logger
_logger = get_logger('servidor_red', 'INFO')

# WebSocket server (mismo proceso, puerto 5556)
try:
    from . import ws_server as _ws
...
HTTP_BIND_HOST = os.getenv("TICKETS_HTTP_BIND_HOST", "").strip()
WS_BIND_HOST = os.getenv("TICKETS_WS_BIND_HOST", "").strip()
REQUIRE_API_KEY = os.getenv("TICKETS_HTTP_REQUIRE_API_KEY", "0").strip() == "1"
API_KEY_HEADER = os.getenv("TICKETS_HTTP_API_KEY_HEADER", "X-Tickets-Key").strip() or "X-Tickets-Key"
API_KEY = os.getenv("TICKETS_HTTP_API_KEY", "").strip()
...
CORS_ALLOWED_ORIGINS = tuple(
    ...
    for origin in os.getenv(
        "TICKETS_HTTP_CORS_ORIGINS",
        "http://localhost:5173,http://127.0.0.1:5173",
    ).split(",")
    if origin.strip()
)
DISCOVERY_CACHE_TTL = int(os.getenv("TICKETS_DISCOVERY_CACHE_TTL", "30"))
DISCOVERY_MAX_WORKERS = int(os.getenv("TICKETS_DISCOVERY_MAX_WORKERS", "24"))
EQUIPO_DB_SYNC_MIN_INTERVAL = int(os.getenv("TICKETS_EQUIPO_DB_SYNC_MIN_INTERVAL", "15"))
...
# Cache del descubrimiento de servidor en red para evitar escaneos frecuentes.
_cache_descubrimiento_servidor: Dict[str, Any] = {"ts": 0.0, "resultado": None}
_lock_cache_descubrimiento = threading.Lock()

# Evita sincronizaciones muy frecuentes de un mismo equipo a BD.
_ult_sync_equipo_db: Dict[str, float] = {}
_lock_sync_equipo_db = threading.Lock()
...
def _origen_permitido(origen: str) -> bool:
    ...
    if "*" in CORS_ALLOWED_ORIGINS:
        return True
    return origen in CORS_ALLOWED_ORIGINS


def _resumen_key(key: str) -> str:
    if not key:
        return "(vacía)"
    if len(key) <= 6:
        return "*" * len(key)
...
def _validar_config_seguridad_inicio() -> None:
    ...
    if not CORS_ALLOWED_ORIGINS:
        advertencias.append("TICKETS_HTTP_CORS_ORIGINS está vacío (clientes web podrían fallar).")

    if not REQUIRE_API_KEY:
        advertencias.append("API key HTTP deshabilitada (TICKETS_HTTP_REQUIRE_API_KEY=0).")
        if STRICT_SECURITY or TICKETS_MODE == "produccion":
            errores.append("En producción se exige API key HTTP (TICKETS_HTTP_REQUIRE_API_KEY=1).")

    if REQUIRE_API_KEY:
        if len(API_KEY) < 24:
            advertencias.append(f"API key HTTP débil: {_resumen_key(API_KEY)}")
            if STRICT_SECURITY or TICKETS_MODE == "produccion":
                errores.append("API key HTTP debe tener al menos 24 caracteres en producción.")

    if (HTTP_BIND_HOST in {"", "0.0.0.0"}) and (STRICT_SECURITY or TICKETS_MODE == "produccion"):
        advertencias.append("HTTP bind amplio; se recomienda IP específica de la LAN.")
    ...
    if errores:
        ...
        raise RuntimeError("Configuración de seguridad no cumple requisitos de producción")
...
ARCHIVO_EQUIPOS_APROBADOS = RUNTIME_DIR / "equipos_aprobados.json"
...
class TicketRequestHandler(BaseHTTPRequestHandler):
    ...
    def _api_key_autorizada(self) -> bool:
        if not REQUIRE_API_KEY:
            return True
        if not API_KEY:
            return False
        return self.headers.get(API_KEY_HEADER, "") == API_KEY

    def _verificar_autorizacion_post(self) -> bool:
        if self.command != 'POST':
            return True

        if not self._api_key_autorizada():
            self._enviar_json(401, {
                "error": "unauthorized",
                "message": f"API key inválida o ausente. Use header {API_KEY_HEADER}.",
            })
            return False
        return True
    ...
    def do_GET(self):
        ...
        # === ENDPOINTS DE ENLACE ===
            ...
            try:
                gestor = _obtener_gestor_tickets()
                health = {
                    "status": "healthy" if gestor else "degraded",
                    "timestamp": datetime.now().isoformat(),
                    "equipo_count": len(obtener_equipos_online()),
                    "websocket_available": WS_SERVER_DISPONIBLE,
                    "notifications_available": NOTIFICACIONES_DISPONIBLES,
                    "request_queue_size": _cola_mensajes.qsize(),
                    "active_connections": _conexiones_activas
                }
                self._enviar_json(200, health)
            except Exception as e:
                log_error(_logger, f"Error en health check: {e}", exception=e)
                self._enviar_json(503, {"status": "unhealthy", "error": str(e)})
    ...
    def do_POST(self):
        ...
        if not self._verificar_autorizacion_post():
            return
        ...
        # Evitar I/O de consola en cada request para no penalizar throughput
    ...
    def _crear_ticket(self, datos: dict):
        try:
            ...
            # Validación de entrada en servidor (defensa en profundidad)
            ...
            if not es_valido:
                self._enviar_json(400, {"error": error})
                return

            # Sanitización previa a persistencia
            usuario_ad = InputValidator.sanitize_string(datos.get("usuario_ad", ""), max_length=100)
            hostname = InputValidator.sanitize_string(datos.get("hostname", ""), max_length=100)
            categoria = InputValidator.sanitize_string(datos.get("categoria", "Otro"), max_length=50)
            descripcion = InputValidator.sanitize_string(
                datos.get("descripcion", ""),
                max_length=InputValidator.MAX_DESCRIPCION,
                allow_newlines=True,
            )
            prioridad = InputValidator.sanitize_string(datos.get("prioridad", "Media"), max_length=20)
            ...
            # Mantener respuesta ligera en hot path
            ticket.setdefault("posicion_cola", 0)
            ticket.setdefault("mensaje_sistema", "Ticket creado")
            ticket.setdefault("tiempo_espera_estimado", 0)
...
def _guardar_equipo_db(equipo: dict):
    ...
    if mac:
        with _lock_sync_equipo_db:
            ultima = _ult_sync_equipo_db.get(mac, 0.0)
            if ahora - ultima < max(1, EQUIPO_DB_SYNC_MIN_INTERVAL):
                return
            _ult_sync_equipo_db[mac] = ahora
...
def iniciar_servidor(puerto: int = PUERTO_HTTP,
    ...
    try:
        # Backup diario no bloqueante al inicio del servicio
        threading.Thread(target=auto_backup_diario, daemon=True, name="Auto-Backup").start()

        ip = host or HTTP_BIND_HOST or obtener_ip_local()
        ...
        if WS_SERVER_DISPONIBLE:
            ws_host = WS_BIND_HOST or ip
            _ws.iniciar_ws_server(puerto=5556, host=ws_host)
...
CLIENT_API_KEY_HEADER = os.getenv("TICKETS_HTTP_API_KEY_HEADER", API_KEY_HEADER).strip() or API_KEY_HEADER
CLIENT_API_KEY = os.getenv("TICKETS_HTTP_API_KEY", "").strip()


def _headers_json_cliente() -> Dict[str, str]:
    headers = {'Content-Type': 'application/json'}
    if CLIENT_API_KEY:
        headers[CLIENT_API_KEY_HEADER] = CLIENT_API_KEY
    return headers
...
def buscar_servidor_en_red(puerto: int = PUERTO_HTTP, timeout: int = 1) -> Optional[Dict]:
    ...
    # Usar cache corto para evitar escaneos de red repetidos en intervalos pequeños.
    ahora = time.time()
    with _lock_cache_descubrimiento:
        if (
            _cache_descubrimiento_servidor["resultado"]
            and ahora - float(_cache_descubrimiento_servidor["ts"]) < max(1, DISCOVERY_CACHE_TTL)
        ):
            return _cache_descubrimiento_servidor["resultado"]
    ...
    if encontrado["ip"]:
        with _lock_cache_descubrimiento:
            _cache_descubrimiento_servidor["ts"] = time.time()
            _cache_descubrimiento_servidor["resultado"] = encontrado.copy()
        return encontrado
    
    # Si no, escanear todo el rango
    with ThreadPoolExecutor(max_workers=max(4, DISCOVERY_MAX_WORKERS)) as executor:
        executor.map(verificar_ip, [f"{base}.{i}" for i in range(1, 255)])

    resultado = encontrado if encontrado["ip"] else None
    with _lock_cache_descubrimiento:
        _cache_descubrimiento_servidor["ts"] = time.time()
        _cache_descubrimiento_servidor["resultado"] = resultado.copy() if resultado else None
    return resultado
