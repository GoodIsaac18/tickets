...
import src.apps.emisora.app  # noqa: F401
...
if __name__ == "__main__":
    try:
        ...
        _log_startup_fatal("kubito", exc)
        raise SystemExit(1)

    if os.getenv("TICKETS_SMOKE_STARTUP", "0") == "1":
        print("[STARTUP][SMOKE] kubito preflight OK")
        raise SystemExit(0)

    try:
        runpy.run_module("src.apps.emisora.app", run_name="__main__")
    except Exception as exc:
        print(f"[STARTUP][FATAL] kubito: {exc}")
        _log_startup_fatal("kubito", exc)
        raise SystemExit(1)
